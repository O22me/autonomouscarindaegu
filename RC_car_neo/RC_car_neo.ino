...
// 뒷바퀴 모터회전
void SetSpeed(float speed)                //-1(후진) ~ 0(정지) ~ 1(전진)
{
  ...
  {
    if (punch_time > 0) //기본값 50 : 정지마찰력 극복시간
    {
      if (speed > 0)
      {
...
void loop() {
  ...
  //center 감지거리가 50이하면 정지
  if (center <= 50) { 
    compute_steering = 0; //바퀴정렬
    compute_speed = 0;    //속도 0
  }
  
  //detect_ir = true, 양쪽 IR센서 모두 true면 직진
  ...
  /*
  Serial.print("rightspace_open : ");
  Serial.print(rightspace_open);
  Serial.print("        rightspace_counter : ");
  Serial.println(rightspace_counter);
  */
}
