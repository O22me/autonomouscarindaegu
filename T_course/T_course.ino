...
//주행코스 별 체크 : boolean
boolean rightspace_complete = true;
boolean T_course_complete = true;
boolean Avoidance_complete = true;

// 1 : 시작선 / 2, 3 : 교차로 / 4 : T자 주차 / 5 : 회피주행 / 6 : 종료주차
int stop_line_count = 5;
...
void loop() {
  ...
  // 전방에 장애물   
    ...
    else if (stop_line_count == 5 && (Avoidance_complete == false)) {
      compute_steering = -0.8;
  ...
  // 양쪽 차선이 모두 검출된 경우(정지선)
    ...
    if (stop_line_count == 4) {
      T_course();
      Serial.print("T_course() complete. stop_line_count : ");
      Serial.println(stop_line_count);
    }
    //loop문 해제를 위한 약간의 전진
    SetSpeed(1);
